...
export default function CityEditor() {
    ...
    const paintTile = useCallback((cx: number, cy: number, erase: boolean) => {
        setLevelData(prev => {
            if (!prev) return null;
            const newData = { ...prev };
            let modified = false;
    ...
    const interpolateLine = useCallback((x0: number, y0: number, x1: number, y1: number, erase: boolean) => {
    ...
    const handleCanvasInteraction = useCallback((e: React.MouseEvent) => {
        ...
        if (isDrawing && lastGridPosRef.current) {
            interpolateLine(lastGridPosRef.current.x, lastGridPosRef.current.y, gx, gy, isRightClick);
        } else if (e.type === 'mousedown' || isDrawing) {
            paintTile(gx, gy, isRightClick);
        }
        if (isDrawing) {
